...
function [x_end,y_end,z_end,alpha,beta,gamma,P,RotationM] = FK_7DOF_FB(L0,L1,L2,L3,L4,L5,x_base,y_base,z_base,JointTheta_j)

%D-H表訂完後的定值
ALPHA=[0.5*pi -0.5*pi 0.5*pi 0 0.5*pi -0.5*pi 0 -0.5*pi 0.5*pi -0.5*pi];   %DH中的alpha  沿著x轉的角度 
THETHA_HOME=[0 0.5*pi -0.5*pi 0.5*pi 0 0 0 0.5*pi -0.5*pi 0];%DH中的theta  沿著Z轉的角度

d_Axis_j=[0 L0 0 L1 0 0 L4 0 0 0];%軸向距d
a_Radia_j=[0 0 0 0 -L2 L3 0 0 0 L5];%徑向距a
...
Theta_j(10)=THETHA_HOME(10)+JointTheta_j(7); %R-END
...
T7_8=DH_HomoTran(d_Axis_j(10),Theta_j(10),a_Radia_j(10),ALPHA(10));% end
